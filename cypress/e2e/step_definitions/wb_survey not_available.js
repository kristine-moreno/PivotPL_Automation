...
When('I login my valid WB survey_id and student_id', () => {
    ...
    surveyId.forEach(testSurvey=>{
        ...
        studentId.forEach(testStudent=>{
            ...
            cy.get(schedSurvey.wereSorryLabel,{timeout:5000}).should('be.visible');
            cy.get(schedSurvey.notOpenLabel,{timeout:5000}).should('be.visible');
            cy.get(schedSurvey.centerImage,{timeout:5000}).should('be.visible');
            cy.get(schedSurvey.notifOpenLabel,{timeout:5000}).should('be.visible');
            cy.get(schedSurvey.notAvailableLogout,{timeout:5000}).should('be.visible');
            cy.log("Elements are available")

            //click logout button
            cy.get(schedSurvey.notAvailableLogout,{timeout:5000}).click();
